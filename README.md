...
**附件存储规则**：所有附件统一存放在 `Attachments/` 目录下，每个页面的附件保存在 `{页面名称}_{日期}_attachments/` 子文件夹中，便于管理和查找。

## 支持的 Block 类型

基础文本：段落、标题（H1-H4）、**折叠标题（HTML 可折叠）**、无序/有序列表、**折叠列表（HTML 可折叠）**、待办事项、引用、分割线、Callout

富文本：粗体、斜体、删除线、下划线、行内代码、高亮、颜色、链接

媒体嵌入：图片（下载到 `Attachments/页面专属文件夹`）、视频、文件、PDF、书签、Embed、Figma、音频、Tweet、Gist
...
> **注意**：折叠标题和折叠列表会使用 HTML `<details>` 标签保持可折叠功能。
